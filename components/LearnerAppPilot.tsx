...
function ImageHero({ compact = false }: { compact?: boolean }) {
...
export default function LearnerAppPilot() {
  ...
  const enterPilot = () => { if (!canEnter) return; window.localStorage.setItem(TEAM_STORAGE_KEY, team.trim()); setTeam(team.trim()); setScreen(1); };
  const changeTeam = () => { window.localStorage.removeItem(TEAM_STORAGE_KEY); setTeam(""); reset(); setScreen(0); };
