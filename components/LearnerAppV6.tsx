...
const secondOptions: { value: SecondChoice; title: string; desc: string; tone: "blue" | "teal" | "purple" }[] = [
  { value: "기존 유지", title: "처음 판단의 방향을 유지", desc: "돌발상황을 보아도 1차 판단의 큰 방향은 그대로 둡니다.", tone: "blue" },
  { value: "일부 수정", title: "방향은 유지하되 조건을 조정", desc: "핵심 방향은 살리되 범위·순서·설명 방식을 수정합니다.", tone: "teal" },
  { value: "판단 변경", title: "처음 판단과 다른 방향 선택", desc: "새로운 변수가 결정적으로 커져 판단 방향을 바꿉니다.", tone: "purple" },
];
...
function Shell({ children, step }: { children: React.ReactNode; step: number }) {
  ...
  return (
    <div className="mx-auto flex min-h-[840px] w-full max-w-[430px] flex-col overflow-hidden rounded-[2.4rem] border border-slate-200 bg-slate-50 shadow-2xl">
      <div className="bg-white/90 px-5 pb-2 pt-5 backdrop-blur">
        <div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-500">9:41</span><b className="text-sm text-slate-800">업무관리 Decision Journey</b><span className="relative h-8 w-8 rounded-full bg-gradient-to-br from-blue-100 to-teal-100"><span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">2</span></span></div>
        <div className="mt-3 flex justify-center gap-2">{labels.map((_, i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${i <= step ? "bg-blue-600" : "bg-slate-200"}`} />)}</div>
      </div>
...
function SecondDecisionCard({ option, selected, onClick }: { option: { value: SecondChoice; title: string; desc: string; tone: "blue" | "teal" | "purple" }; selected: boolean; onClick: () => void }) {
  const selectedClass = option.tone === "blue" ? "border-blue-400 bg-blue-50 ring-blue-100" : option.tone === "teal" ? "border-teal-400 bg-teal-50 ring-teal-100" : "border-violet-400 bg-violet-50 ring-violet-100";
  const badge = option.tone === "blue" ? "bg-blue-600" : option.tone === "teal" ? "bg-teal-600" : "bg-violet-600";
  return <button onClick={onClick} className={`w-full rounded-3xl border p-4 text-left transition ${selected ? `${selectedClass} ring-4` : "border-slate-200 bg-white"}`}><div className="flex items-center gap-3"><div className={`flex min-h-12 w-20 shrink-0 items-center justify-center rounded-2xl px-2 text-center text-sm font-extrabold leading-5 text-white ${badge}`}>{option.value}</div><div className="min-w-0 flex-1"><div className="text-base font-extrabold leading-6 text-slate-800">{option.title}</div><div className="mt-1 text-sm leading-6 text-slate-500">{option.desc}</div></div><div className={`h-5 w-5 shrink-0 rounded-full border-2 ${selected ? "border-blue-600 bg-blue-600" : "border-slate-300"}`} /></div></button>;
}

export default function LearnerAppV6() {
  ...
  return <Shell step={step}><Nav screen={screen} nextDisabled={nextDisabled} onPrev={prev} onHome={home} onNext={navNext} label={navLabel} />
    {screen === 0 && <div className="space-y-6"><div className="rounded-[2rem] bg-gradient-to-br from-blue-50 via-white to-teal-50 p-6"><div className="text-lg font-extrabold text-blue-600">업무관리</div><h1 className="mt-2 text-4xl font-black leading-tight text-slate-900">Decision<br />Journey</h1><p className="mt-4 text-sm leading-7 text-slate-500">실제 업무 딜레마를 따라 판단을 확장해보세요.</p><div className="mt-6 h-40 rounded-3xl bg-gradient-to-br from-sky-100 to-blue-50 p-4"><div className="ml-auto h-24 w-24 rounded-full bg-white/80 shadow-inner" /><div className="mt-2 h-8 w-40 rounded-full bg-blue-100" /></div></div><Card className="space-y-4 p-5"><TextInput label="세션 코드" placeholder="예) ABC123" value={code} onChange={setCode} /><TextInput label="팀명 / 이름" placeholder="예) 새벽등대팀 / 홍길동" value={team} onChange={setTeam} />{!canEnter && <RequiredMessage>세션 코드와 팀명/이름을 모두 입력해주세요.</RequiredMessage>}<PrimaryButton onClick={() => canEnter && setScreen(1)} disabled={!canEnter}>대시보드로 이동</PrimaryButton></Card></div>}
    {screen === 1 && <div className="space-y-5"><div className="rounded-[2rem] bg-gradient-to-br from-blue-50 via-white to-teal-50 p-5"><div className="text-sm font-extrabold text-blue-600">업무관리</div><h2 className="mt-1 text-3xl font-black leading-tight text-slate-900">{team || "홍길동"} 님,<br />오늘의 판단 여정을<br />시작해볼까요?</h2><p className="mt-3 text-sm text-slate-500">🔥 꾸준함이 리더십을 만듭니다.</p></div><Card className="p-5"><div className="grid grid-cols-[120px_1fr] gap-4"><div className="flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-violet-500 p-2"><div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white"><b className="text-3xl text-slate-900">78%</b><span className="text-xs text-slate-500">여정 완료</span></div></div><div><Pill>진행 중 라운드</Pill><h3 className="mt-3 text-lg font-black text-slate-900">라운드 {round.id} · {round.title}</h3><p className="mt-2 text-sm text-slate-500">총 5라운드 중 2라운드 진행 중</p><button onClick={() => startRound(roundIndex)} className="mt-4 w-full rounded-2xl bg-blue-600 px-4 py-3 text-sm font-extrabold text-white">라운드 시작하기 →</button></div></div></Card><div className="grid grid-cols-2 gap-3"><Card className="p-4"><Pill tone="teal">오늘의 목표</Pill><p className="mt-3 text-base font-black text-slate-900">핵심 판단 3회 완료하기</p><div className="mt-4 h-3 rounded-full bg-slate-100"><div className="h-3 w-2/3 rounded-full bg-teal-500" /></div><p className="mt-2 text-xs text-slate-500">2 / 3 · 목표 달성 시 30P</p></Card><Card className="p-4"><Pill tone="purple">오늘의 미션</Pill><p className="mt-3 text-base font-black leading-7 text-slate-900">다양한 관점으로<br />의견을 수집해보세요.</p><p className="mt-3 text-sm font-bold text-blue-600">힌트 보기 →</p></Card></div><Card className="p-4"><div className="flex items-center justify-between"><b className="text-slate-900">나의 의사결정 로그북</b><span className="text-sm font-bold text-blue-600">전체 보기 →</span></div><div className="mt-4 grid grid-cols-3 gap-3 text-center"><div><b className="text-2xl text-slate-900">18회</b><p className="text-xs text-slate-500">누적 판단</p></div><div><b className="text-2xl text-slate-900">82%</b><p className="text-xs text-slate-500">평균 확신도</p></div><div><b className="text-2xl text-blue-600">Lv.2</b><p className="text-xs text-slate-500">판단 확장가</p></div></div></Card><div><div className="mb-3 flex items-center justify-between"><b className="text-slate-900">라운드 선택</b><span className="text-sm font-bold text-blue-600">전체 보기 →</span></div><div className="grid grid-cols-2 gap-3">{rounds.slice(0,4).map((r,i)=><button key={r.id} onClick={()=>startRound(i)} className={`rounded-3xl border p-4 text-left ${i===roundIndex?"border-blue-400 bg-blue-50 ring-4 ring-blue-100":"border-slate-100 bg-white"}`}><Pill tone={i===0?"teal":i===roundIndex?"blue":"gray"}>라운드 {r.id}</Pill><p className="mt-3 text-sm font-black leading-6 text-slate-800">{r.title}</p><p className="mt-3 text-xs text-slate-500">{i===0?"완료":i===roundIndex?"진행 중":"잠김"}</p></button>)}</div></div><div className="rounded-3xl bg-slate-900 p-5 text-white"><b>김박사의 한마디</b><p className="mt-2 text-sm leading-6 text-slate-200">좋은 판단은 경험에서 나오고, 탁월한 판단은 성찰에서 나옵니다.</p></div></div>}
    ...
    {screen === 4 && <div className="space-y-4"><div className="text-center"><Pill>1차 판단</Pill></div><button onClick={() => setFirstChoice("A")} className={`w-full rounded-3xl border p-4 text-left ${firstChoice === "A" ? "border-blue-400 bg-blue-50 ring-4 ring-blue-100" : "border-slate-200 bg-white"}`}><b>A</b><p className="mt-2 font-bold">{round.optionA}</p></button><button onClick={() => setFirstChoice("B")} className={`w-full rounded-3xl border p-4 text-left ${firstChoice === "B" ? "border-teal-400 bg-teal-50 ring-4 ring-teal-100" : "border-slate-200 bg-white"}`}><b>B</b><p className="mt-2 font-bold">{round.optionB}</p></button><TextArea label="선택 이유" placeholder="선택 이유를 적어주세요." value={firstReason} onChange={setFirstReason} /><TextArea label="우려되는 점" placeholder="우려되는 점을 적어주세요." value={concern} onChange={setConcern} />{!canFirst && <RequiredMessage>A/B 선택, 선택 이유, 우려되는 점을 모두 입력해주세요.</RequiredMessage>}<PrimaryButton onClick={next} disabled={!canFirst}>돌발상황 확인하기</PrimaryButton></div>}
