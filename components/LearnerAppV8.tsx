...
function HeroGraphic() {
  return (
    <div className="relative mt-5 h-48 overflow-hidden rounded-[2rem] bg-gradient-to-br from-sky-100 via-white to-blue-100">
      <div className="absolute inset-x-0 bottom-0 h-20 rounded-t-[70%] bg-gradient-to-r from-blue-100 to-teal-100" />
      <div className="absolute bottom-8 right-8 h-24 w-32 rounded-[2rem] bg-slate-700/20" />
      <div className="absolute bottom-12 right-14 h-28 w-12 rounded-t-full bg-white shadow-lg" />
      <div className="absolute bottom-24 right-16 h-8 w-8 rounded-full bg-blue-600" />
      <div className="absolute bottom-8 left-8 h-2 w-40 rotate-[-12deg] rounded-full bg-white/80" />
      <div className="absolute left-8 top-8 rounded-full bg-white/70 px-3 py-1 text-xs font-bold text-blue-700 shadow-sm">판단 여정</div>
    </div>
  );
}
  ...
  return (
    <div className="flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 via-blue-500 to-violet-500 p-2 shadow-lg shadow-blue-100">
      <div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white">
        ...
        <span className="text-xs font-bold text-slate-500">완료</span>
      </div>
    </div>
  );
}
...
function RoundMiniCard({ title, index, active, done, onClick }: { title: string; index: number; active?: boolean; done?: boolean; onClick: () => void }) {
  const icon = ["📋", "👥", "💻", "☕"][index % 4];
  return (
    <button onClick={onClick} className={`min-h-[150px] rounded-3xl border p-4 text-left shadow-sm transition ${active ? "border-blue-400 bg-blue-50 ring-4 ring-blue-100" : "border-slate-100 bg-white"}`}>
      <div className="flex items-start justify-between">
        <Pill tone={done ? "teal" : active ? "blue" : "gray"}>라운드 {index + 1}</Pill>
        <span className="text-2xl">{icon}</span>
      </div>
      <p className="mt-4 text-sm font-black leading-6 text-slate-800">{title}</p>
      <div className="mt-4 flex items-center gap-1">{[0, 1, 2, 3, 4].map((i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${done || (active && i < 2) ? "bg-blue-600" : "bg-slate-200"}`} />)}</div>
...
export default function LearnerAppV8() {
