...
function Shell({ children, step }: { children: React.ReactNode; step: number }) {
  const labels = ["입장", "상황", "1차", "돌발", "2차", "결정"];
  return (
    <div className="mx-auto flex min-h-[820px] w-full max-w-[420px] flex-col overflow-hidden rounded-[2.2rem] border border-slate-200 bg-slate-50 shadow-2xl">
      <div className="bg-white px-5 pb-2 pt-5">
        ...
        <div className="flex justify-center gap-2 py-3">{[0, 1, 2, 3, 4, 5, 6].map((i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${i <= step ? "bg-blue-600" : "bg-slate-200"}`} />)}</div>
      </div>
      <div className="flex-1 overflow-y-auto px-5 py-5">{children}</div>
      <div className="grid grid-cols-6 gap-1 border-t border-slate-100 bg-white px-3 py-3 text-center text-[10px] text-slate-400">{labels.map((s, i) => <div key={s} className={i === step ? "font-bold text-blue-600" : ""}>●<br />{s}</div>)}</div>
    </div>
  );
}
...
function SecondDecisionCard({ option, selected, onClick }: { option: { value: SecondChoice; title: string; desc: string; tone: "blue" | "teal" | "purple" }; selected: boolean; onClick: () => void }) {
  const border = option.tone === "blue" ? "border-blue-400 bg-blue-50 ring-blue-100" : option.tone === "teal" ? "border-teal-400 bg-teal-50 ring-teal-100" : "border-violet-400 bg-violet-50 ring-violet-100";
  const badge = option.tone === "blue" ? "bg-blue-600" : option.tone === "teal" ? "bg-teal-600" : "bg-violet-600";
  return (
    <button onClick={onClick} className={`w-full rounded-3xl border p-4 text-left transition ${selected ? `${border} ring-4` : "border-slate-200 bg-white"}`}>
      <div className="flex items-center gap-3">
        <div className={`flex min-h-12 w-20 shrink-0 items-center justify-center rounded-2xl px-2 text-center text-sm font-extrabold leading-5 text-white ${badge}`}>{option.value}</div>
        <div className="flex-1 min-w-0"><div className="text-base font-extrabold leading-6 text-slate-800">{option.title}</div><div className="mt-1 text-sm leading-6 text-slate-500">{option.desc}</div></div>
        <div className={`h-5 w-5 shrink-0 rounded-full border-2 ${selected ? "border-blue-600 bg-blue-600" : "border-slate-300"}`} />
      </div>
    </button>
  );
}

export default function LearnerAppV5() {
