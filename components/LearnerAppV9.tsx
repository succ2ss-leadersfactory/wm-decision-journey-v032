    ...
    <div className="mx-auto flex min-h-[860px] w-full max-w-[430px] flex-col overflow-hidden rounded-[2.4rem] border border-slate-200 bg-slate-50 shadow-2xl">
      <div className="bg-white/95 px-5 pb-2 pt-5 backdrop-blur">
        <div className="flex items-center justify-between">
          ...
          <b className="text-sm text-slate-800">업무관리 Decision Journey</b>
          <span className="relative h-8 w-8 rounded-full bg-gradient-to-br from-blue-100 to-teal-100">
...
function LighthouseHero() {
  return (
    <div className="relative mt-5 h-52 overflow-hidden rounded-[2rem] bg-gradient-to-br from-sky-100 via-white to-blue-100 shadow-inner">
      <div className="absolute inset-x-0 bottom-0 h-24 rounded-t-[70%] bg-gradient-to-r from-blue-100 via-sky-50 to-teal-100" />
      <div className="absolute bottom-8 right-6 h-28 w-36 rounded-[2.2rem] bg-gradient-to-br from-slate-300/60 to-slate-500/30" />
      <div className="absolute bottom-12 right-16 h-30 w-14 rounded-t-full bg-white shadow-xl" />
      <div className="absolute bottom-[105px] right-[74px] h-9 w-9 rounded-full bg-blue-600 shadow-lg shadow-blue-200" />
      <div className="absolute bottom-10 left-8 h-3 w-44 rotate-[-12deg] rounded-full bg-white/90 shadow-sm" />
      <div className="absolute bottom-16 left-16 h-2 w-32 rotate-[-12deg] rounded-full bg-blue-100/80" />
      <div className="absolute left-6 top-6 rounded-full bg-white/80 px-3 py-1 text-xs font-bold text-blue-700 shadow-sm">판단 여정</div>
      <div className="absolute right-5 top-7 rounded-2xl bg-white/75 px-3 py-2 text-xs font-bold leading-5 text-slate-600 shadow-sm">길을 잃지 않도록<br />판단 기준을 세웁니다</div>
    </div>
  );
}
...
function ProgressRing() {
  return (
    <div className="flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 via-blue-500 to-violet-500 p-2 shadow-lg shadow-blue-100">
      <div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white">
        <b className="text-3xl text-slate-900">78%</b>
        <span className="text-xs font-bold text-slate-500">여정 완료</span>
      </div>
    </div>
  );
}
...
function RoundMiniCard({ title, index, active, done, onClick }: { title: string; index: number; active?: boolean; done?: boolean; onClick: () => void }) {
  const icon = ["📋", "👥", "💻", "☕"][index % 4];
  return (
    <button onClick={onClick} className={`min-h-[158px] rounded-3xl border p-4 text-left shadow-sm transition ${active ? "border-blue-400 bg-blue-50 ring-4 ring-blue-100" : "border-slate-100 bg-white"}`}>
      <div className="flex items-start justify-between">
        <Pill tone={done ? "teal" : active ? "blue" : "gray"}>라운드 {index + 1}</Pill>
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-50 text-2xl shadow-inner">{icon}</span>
      </div>
      <p className="mt-4 min-h-[42px] text-sm font-black leading-6 text-slate-800">{title}</p>
      <div className="mt-3 flex items-center gap-1">{[0, 1, 2, 3, 4].map((i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${done || (active && i < 2) ? "bg-blue-600" : "bg-slate-200"}`} />)}</div>
      <p className="mt-3 text-xs font-bold text-slate-500">{done ? "완료" : active ? "진행 중" : "대기"}</p>
    </button>
  );
}
...
export default function LearnerAppV9() {
