...
const reviewItems = ["우리 조직의 맥락이 반영되었는가?", "팀원의 감정과 체면을 고려했는가?", "상사와 타부서의 기대를 놓치지 않았는가?", "실행 비용을 과소평가하지 않았는가?", "실제 팀장이 말할 수 있는 문장인가?"];
...
const secondQuestions: Record<SecondChoice, string[]> = {
  "기존 유지": ["돌발상황을 보았음에도 기존 판단을 유지해도 된다고 보는 가장 강한 근거는 무엇입니까?", "기존 판단을 유지할 때 가장 커지는 위험은 무엇이며, 어떻게 통제하겠습니까?", "기존 판단을 유지한다는 결정을 누구에게, 어떤 첫 문장으로 설명하겠습니까?"],
  "일부 수정": ["무엇은 그대로 유지하고, 무엇은 수정해야 합니까?", "일부 수정으로 새롭게 생기는 비용이나 부담은 무엇입니까?", "수정안을 실행하기 위해 누구와 언제 무엇을 다시 합의해야 합니까?"],
  "판단 변경": ["1차 판단을 바꿔야 한다고 보게 만든 결정적 증거는 무엇입니까?", "판단을 바꿀 때 생기는 신뢰, 설명, 일정상의 비용은 무엇입니까?", "판단 변경을 팀원이나 상사에게 어떻게 설명해야 저항을 줄일 수 있습니까?"],
};
...
const toneMap: Record<Tone, string> = { blue: "bg-blue-50 text-blue-700", teal: "bg-teal-50 text-teal-700", purple: "bg-violet-50 text-violet-700", orange: "bg-orange-50 text-orange-700", green: "bg-emerald-50 text-emerald-700", gray: "bg-slate-50 text-slate-600" };
...
function Shell({ children, step }: { children: React.ReactNode; step: number }) {
  ...
  return <div className="mx-auto flex min-h-[860px] w-full max-w-[430px] flex-col overflow-hidden rounded-[2.4rem] border border-slate-200 bg-slate-50 shadow-2xl"><div className="bg-white/95 px-5 pb-2 pt-5 backdrop-blur"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-500">9:41</span><b className="text-sm text-slate-800">업무관리 Decision Journey</b><span className="relative h-8 w-8 rounded-full bg-gradient-to-br from-blue-100 to-teal-100"><span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">2</span></span></div><div className="mt-3 flex justify-center gap-2">{labels.map((_, i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${i <= step ? "bg-blue-600" : "bg-slate-200"}`} />)}</div></div><div className="flex-1 overflow-y-auto px-5 py-5">{children}</div><div className="grid grid-cols-5 gap-1 border-t border-slate-100 bg-white px-3 py-3 text-center text-[10px] text-slate-400">{["홈", "라운드", "로그북", "코칭", "내 정보"].map((s, i) => <div key={s} className={i === 0 ? "font-bold text-blue-600" : ""}>●<br />{s}</div>)}</div></div>;
}
...
function Info({ title, children, tone = "blue" }: { title: string; children: React.ReactNode; tone?: Tone }) {
  return <Card className="p-4"><span className={`mb-3 inline-flex rounded-full px-3 py-1 text-xs font-extrabold ${toneMap[tone]}`}>{title}</span><div className="text-sm leading-7 text-slate-600">{children}</div></Card>;
}

function JourneyHero() {
  return <div className="relative mt-5 h-52 overflow-hidden rounded-[2rem] bg-gradient-to-br from-sky-100 via-white to-teal-100 shadow-inner"><div className="absolute inset-x-0 bottom-0 h-24 rounded-t-[65%] bg-gradient-to-r from-blue-100 via-sky-50 to-teal-100" /><div className="absolute bottom-5 right-7 h-28 w-36 rounded-[2.1rem] bg-gradient-to-br from-slate-300/70 to-slate-500/30" /><div className="absolute bottom-[58px] right-[76px] h-24 w-12 rounded-t-full bg-white shadow-xl" /><div className="absolute bottom-[130px] right-[82px] h-5 w-5 rounded-full bg-amber-300 shadow-lg shadow-amber-200" /><div className="absolute bottom-[114px] right-[72px] h-5 w-10 rounded-full bg-blue-600" /><div className="absolute bottom-14 left-8 h-3 w-44 -rotate-12 rounded-full bg-white/95 shadow-sm" /><div className="absolute bottom-20 left-14 h-2 w-36 -rotate-12 rounded-full bg-blue-100/90" /><div className="absolute bottom-28 left-8 rounded-2xl bg-white/75 px-3 py-2 text-xs font-bold leading-5 text-slate-600 shadow-sm">선택의 길을 따라<br />판단 기준을 세웁니다</div><div className="absolute left-6 top-6 rounded-full bg-white/85 px-3 py-1 text-xs font-bold text-blue-700 shadow-sm">판단 여정</div></div>;
}

function ProgressRing() {
  return <div className="flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 via-blue-500 to-violet-500 p-2 shadow-lg shadow-blue-100"><div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white"><b className="text-3xl text-slate-900">78%</b><span className="text-xs font-bold text-slate-500">여정 완료</span></div></div>;
}

function RoundMiniCard({ title, index, active, done, onClick }: { title: string; index: number; active?: boolean; done?: boolean; onClick: () => void }) {
  const icon = ["📋", "👥", "💻", "☕"][index % 4];
  return <button onClick={onClick} className={`min-h-[158px] rounded-3xl border p-4 text-left shadow-sm transition ${active ? "border-blue-400 bg-blue-50 ring-4 ring-blue-100" : "border-slate-100 bg-white"}`}><div className="flex items-start justify-between"><Pill tone={done ? "teal" : active ? "blue" : "gray"}>라운드 {index + 1}</Pill><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-50 text-2xl shadow-inner">{icon}</span></div><p className="mt-4 min-h-[42px] text-sm font-black leading-6 text-slate-800">{title}</p><div className="mt-3 flex items-center gap-1">{[0, 1, 2, 3, 4].map((i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${done || (active && i < 2) ? "bg-blue-600" : "bg-slate-200"}`} />)}</div><p className="mt-3 text-xs font-bold text-slate-500">{done ? "완료" : active ? "진행 중" : "대기"}</p></button>;
}

function StepHero({ badge, title, subtitle, icon, tone = "blue" }: { badge: string; title: string; subtitle: string; icon: string; tone?: Tone }) {
...
function FirstOptionCard({ label, title, selected, onClick, tone }: { label: FirstChoice; title: string; selected: boolean; onClick: () => void; tone: "blue" | "teal" }) {
  const selectedClass = tone === "blue" ? "border-blue-400 bg-blue-50 ring-blue-100" : "border-teal-400 bg-teal-50 ring-teal-100";
  const badge = tone === "blue" ? "bg-blue-600" : "bg-teal-600";
...
function SecondDecisionCard({ option, selected, onClick }: { option: { value: SecondChoice; title: string; desc: string; tone: "blue" | "teal" | "purple"; icon: string }; selected: boolean; onClick: () => void }) {
